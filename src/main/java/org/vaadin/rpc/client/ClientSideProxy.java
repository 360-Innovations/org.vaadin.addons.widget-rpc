...
public class ClientSideProxy implements Serializable {
    ...
    private HashMap<String, Method> callHandlers = new HashMap<String, Method>();
        ...
        for (int i = 0; i < pc; i++) {
            ...
            if (callNode.hasAttribute(pn) && callNode.hasAttribute("pt" + i)) {
                ...
                if (pt == ClientSideProxy.PARAM_STRING) {
                    ...
                    params.add(appConn.translateVaadinUri(callNode
                            .getStringAttribute(pn)));
    ...
    public class Transcation {
        ...
        public void commit() {
            ...
            if (nestedLevel > 0) {
                d("Nested transaction commit only to parent. level="
                        + nestedLevel);
                nestedLevel--;
                return;
            } else {
